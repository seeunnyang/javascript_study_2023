//1. 변수생성
const btn2 = document.querySelector('#btn2');
const btn3 = document.querySelector('#btn3');
const btn4 = document.querySelector('#btn4');
const btn5 = document.querySelector('#btn5');
const input_id = document.querySelector('#user_id');
console.log(btn2, btn3, btn4, btn5, input_id); //정상확인
//2. 이벤트 작성
//btn3 클릭했을 때 경고창 띄우기
//객체.이벤트리스너('이벤트종류',함수(){실행결과})
btn3.addEventListener('click',function(){
    window.alert('error');
})//객체.이벤트리스너('click',function(){window.alert('error')})
btn4.addEventListener('mouseover',function(){
    this.style.backgroundColor = 'skyblue';
    //this 현재 이벤트 객체 대상 == btn4
    //현재대상.style.css속성 = '대입값'
})
btn5.addEventListener('click',function(){
    btn4.style.transform = 'skewX(10deg)';
    btn3.style.backgroundColor = 'yellow';
    btn2.innerHTML = 'button2';
})//btn5를 클리했을 때 btn4,btn3,btn2 변형
input_id.addEventListener('focus',function(){
    this.style.backgroundColor = 'lime';
}) // 포커스했을 때 배경색 변함
input_id.addEventListener('blur',function(){
    this.style.backgroundColor = 'gray';
}) //포커스에서 벗어났을 때 배경색 변함 (blur=포커스반대)


//변수.addEventListener('이벤드종류',function(){실행});
//변수.addEventListener('이벤트종류',function(){실행});
//변수.addEventListener('이벤트종류',function(){실행});
//변수.addEventListener('이벤트종류',function(){실행});
//변수.addEventListener('이벤트종류',function(){실행});
//변수.addEventListener('이벤트종류',function(){실행});
//변수.addEventListener('이벤트종류',function(){실행});